...
import { defaultExecutor, doctor as dependencyDoctor, extractVersion, probeSubagentDepth, } from "./deps.js";
import { discoverAvailableModels } from "./model-config.js";
import { CODING_ROLES, PACKAGE_SKILL_NAMES, roleRequirementIssues, validateZotPilotPolicy, } from "./register.js";
import { deriveRoutes, ROLES } from "./routes.js";
...
const ROLE_MODES = new Set(["primary", "subagent", "all"]);
/**
...
function validateDefaults(raw) {
    ...
    for (const role of ROLES) {
        ...
        if (typeof mode !== "string" || !ROLE_MODES.has(mode)) {
...
async function collectPackageFindings(packageRoot, fileOps) {
    const findings = [];
...
// ---------------------------------------------------------------------------
function routeFinding(route, discovered) {
 ...
 */
function subagentDepthFinding(probe) {
...
function dependencyFindings(deps) {
    return [
        {
 ...
 * configuration.
 */
async function probeZotPilotCli(executor) {
    try {
        const result = await executor("zotpilot", ["--version"]);
        const version = extractVersion(result.stdout);
        if (!version)
            return { found: false, version: null, reason: "probe output not parseable" };
...
function zotPilotCliFinding(probe) {
 ...
 */
function zotPilotMcpFindings(deps) {
    const findings = [];
...
function parseSkillFrontmatter(text) {
    ...
    if (!match?.[1])
        return { error: "frontmatter missing" };
    const block = match[1];
 ...
 */
async function collectSkillFindings(packageRoot, fileOps) {
    const findings = [];
    const failures = [];
 ...
 */
async function collectWikiFindings(packageRoot, fileOps) {
    const findings = [];
    const stat = fileOps.stat ?? ((path) => fsStat(path));
    const access = fileOps.access ?? ((path, mode) => fsAccess(path, mode));
    const missingAssets = [];
    for (const asset of WIKI_PIPELINE_ASSETS) {
        ...
        try {
            ...
            if (!info.isFile())
                missingAssets.push(`${asset} (not a file)`);
        }
        catch (error) {
...
export async function runDoctor(options = {}) {
    ...
    const discovery = options.discovery ?? ((worktreePath) => discoverAvailableModels(worktreePath));
    ...
    if (routes && discovered) {
        ...
        for (const route of routes)
            findings.push(...routeFinding(route, byId));
    }
    else {
        findings.push({
            severity: "SKIP",
 ...
 */
export function doctorExitCode(findings) {
