...
const WINDOWS_SHIM_EXTENSIONS = [".cmd", ".bat", ".exe"];
function parsePathExt(pathExt) {
    if (!pathExt)
        return [...WINDOWS_SHIM_EXTENSIONS];
    return pathExt
        .split(";")
        .map((e) => e.trim())
        .filter(Boolean)
 ...
 */
export function resolveCommand(cmd, pathEnv, pathExt, platform, probe) {
 ...
 */
export function buildComSpecArgs(command, args) {
 ...
 * configuration.
 */
export async function probeSubagentDepth(executor, worktree) {
    try {
        const result = await executor("opencode", ["debug", "config"], worktree ? { cwd: worktree } : undefined);
